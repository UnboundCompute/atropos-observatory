...
export default function Search() {
  ...
  return <div className="search-wrap"><label htmlFor="catalog-search">Search the model pack <kbd>/</kbd></label><div className="search-box"><svg aria-hidden="true" viewBox="0 0 24 24"><circle cx="10.5" cy="10.5" r="6.5"/><path d="m15.5 15.5 5 5"/></svg><input ref={inputRef} id="catalog-search" role="combobox" aria-haspopup="listbox" aria-expanded={results.length > 0} aria-controls={results.length > 0 ? listId : undefined} aria-activedescendant={active >= 0 ? `${listId}-${active}` : undefined} aria-autocomplete="list" aria-busy={loading} autoComplete="off" value={query} onFocus={loadIndex} onChange={(event) => { loadIndex(); setQuery(event.target.value); setActive(-1); }} onKeyDown={onKeyDown} placeholder="Try child_process.exec, pickle.load, memcpy…" />{query && <button className="search-clear" type="button" onClick={() => { setQuery(''); setActive(-1); }}>Clear</button>}</div>{loading && <p className="search-status" role="status">Loading the local model index…</p>}{error && <p className="search-error" role="status">The local search index could not be loaded. <button className="search-retry" type="button" onClick={loadIndex}>Try again</button> or <Link href="/browse">browse the catalog</Link>.</p>}{hasQuery && !loading && !error && results.length === 0 && <p className="search-status" role="status">No matching model facts. Try a method name, package, language, or role.</p>}{results.length > 0 && <div className="search-results" id={listId} role="listbox" aria-label="Model search results">{results.map((entry, index) => <Link href={`/symbols/${entry.slug}`} id={`${listId}-${index}`} role="option" aria-selected={active === index} key={entry.id}><b>{entry.method}</b><small>{entry.language} · {entry.package || 'stdlib'}</small><span className={`role role-${entry.role}`}>{entry.role}</span></Link>)}</div>}</div>;
}
