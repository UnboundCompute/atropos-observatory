...
export default function FacetExplorer() {
  ...
  useEffect(() => {
    ...
    setRole(params.get('role') || 'all'); setLanguage(params.get('language') || 'all'); setKind(params.get('kind') || 'all'); setCwe(params.get('cwe') || 'all'); setStatus(params.get('status') || 'all'); setQuery(params.get('q') || ''); setHydrated(true);
  }, []);
  useEffect(() => {
    if (!hydrated) return;
    ...
    const next = params.toString(); window.history.replaceState(null, '', next ? `/browse?${next}` : '/browse');
