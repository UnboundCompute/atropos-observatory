...
export default async function SymbolPage({ params }: { params: Promise<{ slug: string[] }> }) {
  ...
  const entry = facts[0];
  if (!entry) return <main><section className="catalog-intro"><p className="edition">Model not found</p><h1>No matching fact.</h1><Link className="text-link" href="/browse">Return to index</Link></section></main>;
