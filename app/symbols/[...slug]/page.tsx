...
export default async function SymbolPage({ params }: { params: Promise<{ slug: string[] }> }) {
  ...
  const securityGuide = securityGuides.find((guide) => guide.kinds.includes(entry.kind));
  ...
  const related = [...new Map(catalog.filter((item) => item.slug !== key && ((entry.package && item.package === entry.package) || item.kind === entry.kind || (entry.cwe || []).some((cwe) => (item.cwe || []).includes(cwe)))).map((item) => [item.slug, item])).values()].slice(0, 4);
  const relatedLinks = related.map((item) => <Link href={`/symbols/${item.slug}`} key={item.slug}>Related: {item.method}</Link>);
  const traceLinks = casefiles.length
    ? casefiles.map((url) => <a href={url} key={url}>Open related Trace Casefile ↗</a>)
    : [<a href="https://trace.unboundcompute.com/" key="trace">Browse Trace Casefiles <span className="link-note">(no direct mapping yet) ↗</span></a>];
  return <main><section className="detail"><nav className="crumb" aria-label="Breadcrumb"><Link href="/">Observatory</Link><span>/</span><Link href="/browse">Model reference</Link><span>/</span><b aria-current="page">{entry.method}</b></nav><div className="detail-grid"><article><div className={`role role-${entry.role}`}>{[...new Set(facts.map((fact) => fact.role))].join(' · ')}</div><h1>{entry.method}</h1><p className="detail-sub">{entry.package || 'standard library'}{entry.type ? ` · ${entry.type}` : ''}</p><div className={`authority ${reviewed ? 'authority-reviewed' : 'authority-model'}`}><strong>{reviewed ? 'Editorially reviewed overlay' : 'Model fact only'}</strong><span>{reviewed ? `${editorialMeta.applies_to} Reviewed ${editorialMeta.reviewed_at}.` : 'Generated from Atropos; no editorial safety conclusion is attached.'}</span></div>{reviewed && securityGuide && <div className="guide-bridge"><span>Part of the {securityGuide.title.toLowerCase()} guide</span><Link href={`/guide#${securityGuide.slug}`}>Read the full security question and checklist</Link></div>}<div className="callout"><span>{reviewed ? 'Why this is indexed' : 'Model boundary'}</span><p>{summary.summary}</p></div>{safePattern && <div className="callout safe"><span>Safer direction</span><p>{safePattern.safe_pattern}</p></div>}{examples && <div className="example-grid"><div><span>Unsafe shape</span><pre>{examples.unsafe_example || 'See the model boundary above.'}</pre></div><div><span>Safer shape</span><pre>{examples.safe_example || 'Use the constrained alternative described above.'}</pre></div></div>}<section className="fact-register" aria-labelledby="facts-heading"><h2 id="facts-heading">Model facts</h2><table><thead><tr><th scope="col">Role</th><th scope="col">Kind</th><th scope="col">Access path</th><th scope="col">Model ID</th></tr></thead><tbody>{facts.map((fact) => <tr key={fact.id}><td data-label="Role"><span className={`role role-${fact.role}`}>{fact.role}</span></td><td data-label="Kind">{fact.kind}</td><td data-label="Access path"><code>{fact.access_path || 'Not specified'}</code></td><td data-label="Model ID"><code>{fact.id}</code></td></tr>)}</tbody></table></section>{related.length > 0 && <section className="related"><h2>Adjacent model symbols</h2><div>{relatedLinks}</div></section>}{facts.filter((fact) => fact.notes).map((fact) => <div className="callout" key={`${fact.id}-note`}><span>Atropos note / {fact.id}</span><p>{fact.notes}</p></div>)}{notes.filter((note) => note?.limitations).map((note, index) => <div className="callout" key={`limitation-${index}`}><span>Limitation</span><p>{note.limitations}</p></div>)}{references.length > 0 && <div className="references"><span>References</span>{references.map((reference) => <a href={reference} key={reference}>{new URL(reference).hostname} ↗</a>)}</div>}</article><aside><div className="aside-label">Symbol record</div><dl><dt>Language</dt><dd>{entry.language}</dd><dt>Package</dt><dd>{entry.package || 'stdlib'}</dd><dt>Facts</dt><dd>{facts.length}</dd><dt>Confidence</dt><dd>{[...new Set(facts.map((fact) => fact.confidence || 'verified'))].join(', ')}</dd><dt>CWE</dt><dd>{[...new Set(facts.flatMap((fact) => fact.cwe || []))].join(', ') || '—'}</dd><dt>Source file</dt><dd>{entry.source_file}</dd><dt>Ingested revision</dt><dd><code>{stats.source_revision.slice(0, 12)}</code></dd></dl><a className="button" href={`https://github.com/UnboundCompute/atropos/blob/${stats.source_revision}/models/${entry.source_file}`}>Open exact source at ingested revision</a></aside></div><div className="detail-footer"><Link href="/browse">Return to model reference</Link>{traceLinks}</div></section></main>;
}
